...
def get_vgg16_target_layers(model):
    return {
        "early": model.features[2],
        "middle": model.features[14],
        "late": model.features[28],
    }
...
def save_feature_grid(selected_maps: torch.Tensor, layer_name: str, save_path: Path):
    ...
    fig.suptitle(f"VGG16 {layer_name} feature maps (top-{num_maps})", fontsize=12)
    plt.tight_layout()
    fig.savefig(save_path, dpi=200, bbox_inches="tight")
    plt.close(fig)


def generate_vgg16_feature_maps(
    input_csv: str,
    row_index: int = None,
    image_name: str = None,
    top_k: int = 16,
    output_dir: str = "outputs/vgg16_feature_maps",
):
    ...
    target_layers = get_vgg16_target_layers(model)
...
if __name__ == "__main__":
    result = generate_vgg16_feature_maps(
        input_csv="outputs/vgg16_correct_subset.csv",
        row_index=0,
        image_name=None,
        top_k=16,
        output_dir="outputs/vgg16_feature_maps",
    )
